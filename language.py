"""
Language resource module for multilingual support
"""

class LanguageResources:
    """Class for managing text resources for multilingual UI"""
    ...
    resources = {
        ...
        "ko": {
            ...
            # Model list
            "refresh": "새로고침",
            "ollama_connection_error": "Ollama 서버 연결 실패. 기본 모델 목록을 사용합니다.",
            "web_usage_steps": """
        ...
        "en": {
            ...
            # Model list
            "refresh": "Refresh",
            "ollama_connection_error": "Failed to connect to Ollama server. Using default model list.",
            "web_usage_steps": """
    ...
    def get(cls, lang_code, key):
        """Returns text corresponding to the resource key for a specific language"""
        if lang_code not in cls.resources:
            lang_code = "ko"  # Default language is Korean
        
        if key not in cls.resources[lang_code]:
            # If key not found, search in default language
    ...
    @classmethod
    def get_keys(cls, lang_code):
        """Returns list of available keys for a specific language"""
        if lang_code not in cls.resources:
            lang_code = "ko"  # Default language is Korean
        
        return list(cls.resources[lang_code].keys())
